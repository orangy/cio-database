...
private fun encode(data: ByteArray, digits: String = DIGITS_LOWER, maxBytes: Int = data.size): String = buildString(data.size * 2) {
    for (n in 0 .. data.lastIndex.coerceAtMost(maxBytes - 1)) {
...
fun ByteArray.toHexString(maxBytes: Int = this.size): String = encode(this, maxBytes = maxBytes)
